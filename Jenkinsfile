node('docker') {
  ...
  def img = docker.build("gcr.io/nokt-dev/gceme:${env.BUILD_TAG}")
  sh('gcloud docker -a')
  img.push()
  ...
  docker.image('buildpack-deps:jessie-scm').inside {
    ...
    sh('curl -o /usr/bin/kubectl https://storage.googleapis.com/kubernetes-release/release/v1.2.0/bin/linux/amd64/kubectl ; chmod +x /usr/bin/kubectl')
